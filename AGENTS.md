...
Changed behavior requires deterministic coverage in `test/engine.test.js`, with browser checks added where relevant. Responsive panel geometry and the 798 CSS-pixel desktop boundary are covered by `test/layout.spec.js`. Correct affected stale documentation in the same change. Maintain user-facing, workflow-facing, and agent-facing Markdown in `README.md`, `CONTRIBUTING.md`, and `AGENTS.md`. Do not check in generated JSDoc under `docs/` unless a concrete consumer and a reproducible pinned generation command both exist.
